...
contract TodoDebot is Debot, Upgradable {
    ...
    function savePublicKey(string value) public {
        ...
        if (status) {
            ...
            Terminal.print(0, "Checking if you already have a Purchases list ...");
            ...
            Terminal.print(0, format( "Info: your Purchase contract address is {}", m_address));
    ...
    function checkStatus(int8 acc_type) public {
        if (acc_type == 1) { // acc is active and  contract is already deployed
            ...
            Terminal.print(0, format(
                "Deploying new contract. If an error occurs, check if your Purchase contract has enough tokens on its balance"
            ));
            deploy();
    ...
    function updateTask_(string value) public {
        ...
        m_taskId = uint32(num);
        ConfirmInput.get(tvm.functionId(updateTask__),"Is this purchase completed?");
    }

    function updateTask__(bool value) public view {
        optional(uint256) pubkey = 0;
        ITodo(m_address).updatePurchase{
                abiVer: 2,
                extMsg: true,
                sign: true,
                pubkey: pubkey,
    ...
    function deleteTask(uint32 index) public {
        index = index;
        if (m_stat.completeCount + m_stat.incompleteCount > 0) {
            Terminal.input(tvm.functionId(deleteTask_), "Enter purchase number:", false);
        } else {
    ...
    function deleteTask_(string value) public view {
        (uint256 num,) = stoi(value);
        optional(uint256) pubkey = 0;
        ITodo(m_address).deletePurchase{
                abiVer: 2,
                extMsg: true,
                sign: true,
                pubkey: pubkey,
